...
def test_graphiti_fixture_normalization_preserves_temporal_graph_data() -> None:
    memory = normalize_graphiti_record(
        _fixture_record(), scope=MemoryScope(user_id="user-123")
    )

    assert memory.id == "m1"
    assert memory.content == "User prefers Python."
    assert memory.active is True
    assert memory.source_refs[0].transcript_id == "conversation-1"
    assert memory.provenance_status is ProvenanceStatus.VERIFIED
